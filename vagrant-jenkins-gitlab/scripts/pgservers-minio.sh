...
git clone https://github.com/patsevanton/wal-g-rpm.git && cd wal-g-rpm/
cp *.sh /usr/local/bin/ && chown postgres:postgres /usr/local/bin/{wal*,backup*,build*}

yum -y install yum-plugin-copr && yum copr enable -y antonpatsev/wal-g && yum install -y wal-g
mkdir /etc/wal-g.d/
...
export WALE_S3_PREFIX="s3://postgresql" #'postgresql' is bucket name in minio
export AWS_ACCESS_KEY_ID="pgsql_user" # access_key from minio
export AWS_ENDPOINT="http://10.1.42.101:9001" # Minio server and port
