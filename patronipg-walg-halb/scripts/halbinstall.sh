...
listen master_postgres
    ...
    server patpg1_5432 10.1.42.141:5432 maxconn 100 check port 8008
    server patpg2_5432 10.1.42.142:5432 maxconn 100 check port 8008
    server patpg3_5432 10.1.42.143:5432 maxconn 100 check port 8008
...
frontend minio
    bind *:9001
    mode http
    default_backend storage_pool

backend storage_pool
    balance roundrobin
    mode http
    option forwardfor
    server minio01 10.1.42.171:9001 check
    server minio02 10.1.42.172:9001 check
    server minio03 10.1.42.173:9001 check
    server minio04 10.1.42.174:9001 check
EOF
